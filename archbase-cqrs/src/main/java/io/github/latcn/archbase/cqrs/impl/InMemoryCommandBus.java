...
public class InMemoryCommandBus implements ICommandBus {
	...
	private final Map<Class<? extends ICommand>, ICommandHandler> registry = new ConcurrentHashMap<>();
	...
	@Override
	public <R extends IResponse> R dispatch(ICommand command) {
		Class<? extends ICommand> commandType = command.getClass();
		ICommandHandler handler = registry.get(commandType);

		if (handler == null) {
			throw BaseException.of(ErrorCode.HANDLER_NOT_FOUND).set("command", commandType.getName());
		}
		...
		try {
			...
			throw BaseException.wrap(e, ErrorCode.SYSTEM_ERROR).set("command", commandType.getName());
		}
	}
	...
	private static class CommandInvocation<C extends ICommand, R extends IResponse> implements IBusInvocation<R> {
		...
		CommandInvocation(ICommandHandler<C, R> handler, C command, List<IBusInterceptor> interceptors) {
