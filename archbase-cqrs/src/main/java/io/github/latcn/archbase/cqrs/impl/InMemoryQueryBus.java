...
public class InMemoryQueryBus implements IQueryBus {
    private final Map<Class, IQueryHandler> registry = new ConcurrentHashMap<>();
    ...
    public void register(Class queryType, IQueryHandler handler) {
        registry.put(queryType, handler);
    }

    @SuppressWarnings("unchecked")
    @Override
    public <T, R extends IResponse> R dispatch(IQuery<T> query) {
    ...
    private static class QueryInvocation<Q extends IQuery, R extends IResponse> implements IBusInvocation<R> {
        ...
        QueryInvocation(IQueryHandler<Q, R> handler,
                        Q query,
